  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-6">
      ...
      <textarea
        ...
        rows={5}
        className="resize-none rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-900 placeholder-slate-400 focus:border-slate-500 focus:outline-none"
      />
      <button
        type="button"
        onClick={handleParse}
        disabled={!text.trim()}
        className="self-start rounded-lg border border-slate-300 bg-white px-5 py-2 text-sm font-medium text-slate-700 transition-colors hover:border-slate-500 hover:bg-slate-100 hover:text-slate-900 disabled:opacity-40"
      >
        解析文本
      </button>
    </div>
